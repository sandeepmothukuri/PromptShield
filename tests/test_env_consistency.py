...
# Directories that hold no configuration worth scanning.
_EXCLUDED_PARTS = {".git", "node_modules", ".venv", "__pycache__", "docs"}


def _declared_vars() -> dict[str, str]:
    """Variable names in .env.example, with their documented default."""
    out: dict[str, str] = {}
...
def _referenced_vars() -> set[str]:
    """Variables read by os.getenv/environ, ${VAR} in compose, or shell ${VAR}."""
    found: set[str] = set()
    patterns = (
...
def test_every_declared_variable_is_consumed() -> None:
    """No dead config: setting a documented variable must change behaviour."""
...
def test_env_example_is_not_empty() -> None:
    """Guard against the test above passing because the file was gutted."""
...
def test_llm_monitor_forwards_every_proxy_variable() -> None:
    """Variables the proxy reads must be forwarded by the llm-monitor service.

    A variable read by proxy.py but absent from the service environment can only
    be set by editing compose, which defeats the point of .env.
    """
...
def test_declared_variables_are_documented_or_forwarded() -> None:
    """Each declared var should reach a container, a script, or the code."""
...
def test_every_compose_substitution_has_a_default() -> None:
    """The stack must start with no .env present.

    `${VAR}` without a default yields an empty string, which silently produces a
    broken configuration rather than an error.
    """
...
def test_compose_images_are_pinned() -> None:
    """:latest and :main are not reproducible."""
...
def test_compose_declares_no_duplicate_volumes() -> None:
    ...
    # Every named volume referenced by a service must be declared.
    declared = set(names)
    used = set()
...
def test_flag_threshold_is_below_classifier_threshold() -> None:
    """Hunt 2 searches the gap between the two; equal values make it empty."""
...
def test_shipper_registry_is_writable_in_compose() -> None:
    """The audit volume is read-only, so the offset file must live elsewhere.

    Regression guard: the registry once defaulted to <log>.offset inside the
    read-only mount and would have crashed the container on startup.
    """
    ...
    # `command` may be a folded scalar (str) or a list; normalise both.
    raw = svc["command"]
    ...
    # The registry path must not sit under a read-only mount point.
...
def test_healthchecks_exist_for_services_others_depend_on() -> None:
    """A dependent waiting on service_started for a slow service is a race."""
    compose = _compose()
...
def test_hunt2_window_is_populated_by_the_corpus() -> None:
    """The sub-threshold hunt must have something to find.

    The weighted classifier produces a small set of discrete scores, so a
    reporting floor chosen between two of them silently empties the window that
    hunt 2 ("Sub-threshold probing") searches. This evaluates the real classifier
    over the committed corpus and fails if nothing lands in the gap.

    Deliberately does not touch the environment: `classify().score` is
    independent of FLAG_THRESHOLD (only attack_type reporting consults it), so
    the window can be computed from the declared values directly. An earlier
    version set the env var and reloaded the module, which leaked the floor into
...
def test_wazuh_api_credentials_do_not_drift() -> None:
    """The API password is duplicated in two files that nothing links.

    compose passes WAZUH_API_PASSWORD to the manager, while
    config/wazuh_dashboard/wazuh.yml names the account the dashboard logs in
    with. They must agree or the dashboard cannot reach the API. This is a
    manual coupling, so it is checked rather than trusted.
    """
    declared = _declared_vars()
...
def test_indexer_credentials_match_the_shipped_hashes() -> None:
    """`.env.example` claims the demo passwords match the bcrypt hashes.

    Verified with bcrypt rather than asserted by comment. If the hashes are
    regenerated without updating .env.example, the stack will not authenticate
    and the failure will look like a certificate problem.
    """
...
def test_wazuh_api_user_is_not_an_indexer_user() -> None:
    """Guards the distinction documented in .env.example.

    wazuh-wui is a manager REST API account, not an indexer account. If someone
    adds it to internal_users.yml, the two identity systems are being conflated.
    """
    declared = _declared_vars()
    text = (ROOT / "config" / "wazuh_indexer" / "internal_users.yml").read_text(encoding="utf-8")
    hashes = dict(re.findall(r'^([a-z_-]+):\s*\n\s*hash:\s*"([^"]+)"', text, re.M))
...
# Images named in prose as worked examples of something the lab does not ship.
# These are deliberately not in compose; the surrounding text says so.
_ILLUSTRATIVE_IMAGES = {
    # docs/network-detection.md shows how you *would* ship Suricata logs.
    "docker.elastic.co/beats/filebeat:8.13.4",
}

# Paths that do not exist in the repository because the documented one-time
# certificate step creates them. Anything else must be present, because Docker
# auto-creates a missing bind-mount source as a *directory*, which produces a
# confusing failure inside the container rather than at compose time.
_CERT_DIR = "config/wazuh_indexer_ssl_certs"


def test_bind_mount_sources_exist_or_are_generated_certs() -> None:
    """A missing bind-mount source becomes a directory and breaks the container.

    This caught a real defect: wazuh.manager mounted root-ca-manager.pem, which
    the certificate generator never emits, so the manager received a directory
    where it expected a PEM.
    """
    compose = _compose()
    missing = []
    for name, svc in compose["services"].items():
        for mount in svc.get("volumes", []):
            ...
            if src.startswith(f"./{_CERT_DIR}/"):
                continue  # produced by the documented cert step
...
def test_certificate_mounts_reference_generator_outputs() -> None:
    """Every cert mounted must be a file the generator actually emits.

    config/certs.yml names three nodes, so the generator writes root-ca.pem,
    admin(-key), and <node>.pem / <node>-key.pem for each. A mount naming
    anything else will silently become a directory.
    """
...
def test_documented_images_match_the_committed_compose_files() -> None:
    """Docs must not name an image or tag the stack does not use.

    The README once told users to run wazuh/wazuh-certs-generator:4.8.0, which
    does not exist on Docker Hub, so the certificate step failed and the stack
    could never start. This compares documentation against the committed compose
    files rather than querying a registry, so it runs in CI without network.
    """
    ...
    # Every `registry/repo:tag` string mentioned in prose must exist in a compose
    # file. The leading registry segment must contain a dot or be a bare
    # namespace, which keeps volume mounts like `var/log/suricata:ro` out.
    mentioned = re.compile(r"\b([a-z0-9-]+(?:\.[a-z0-9-]+)+/[a-z0-9._/-]+:[A-Za-z0-9._-]+)\b")
    # `:ro` / `:rw` are mount modes, not tags.
    not_a_tag = re.compile(r":(?:ro|rw)$")
    docs = [ROOT / "README.md", *sorted((ROOT / "docs").glob("*.md"))]
    unknown: list[str] = []
    for doc in docs:
        if "captures" in str(doc):
            continue
        for ref in mentioned.findall(doc.read_text(encoding="utf-8")):
            if not_a_tag.search(ref):
                continue
            # An explicitly illustrative snippet (e.g. "here is how you would add
            # Filebeat") is allowed; only stack components must match.
...
def test_shebang_and_executable_bit_agree() -> None:
    """A shebang without the exec bit, or vice versa, is always a mistake.
...
# Drawn mockups of screens. Each must carry both a visible badge and a <desc>,
# so nobody mistakes them for captures from a running lab.
_MOCKUPS = [
    "dashboard-overview",
    "hunting-query",
    "openwebui",
...
def test_mockups_carry_visible_provenance() -> None:
    """A mockup without a label reads as a screenshot.

    These files were previously presented as real output. Each now carries a
    visible badge and a machine-readable <desc>; both are checked because the
    badge is what a human sees and the <desc> is what survives a re-export.
    """
    bad: list[str] = []
    for name in _MOCKUPS:
...
def test_mockup_facts_match_the_repository() -> None:
    """Where a mockup states a checkable fact, it must agree with the repo.

    These images claimed Suricata 7.0.6 with 6 rules (the compose pin is 7.0.17
    and there are 7 signatures) and carried ATT&CK IDs the mapping audit had
    rejected. The numbers are asserted so the drift cannot come back.
    """
    import re as _re
    ...
    # Technique IDs shown in the Wazuh mockup must exist on the real rules.
...
def test_compose_defaults_match_env_example() -> None:
    """`${VAR:-default}` in compose must equal the value in .env.example.

    These are two independent statements of the same default. FLAG_THRESHOLD was
    0.30 in .env.example and 0.65 in compose, so anyone who did not copy
    .env.example silently got the value the derivation had rejected.
    """
    declared = _declared_vars()
    compose = _compose()
        ...
        for entry in svc.get("environment") or []:
            ...
            for var, default in default_pat.findall(entry):
                if var not in declared:
                    continue  # covered by test_every_documented_variable_is_consumed
...
def test_every_compose_variable_is_declared() -> None:
    """The reverse of test_every_declared_variable_is_consumed.

    A variable compose interpolates but .env.example never mentions is invisible
    to whoever copies the file, so they cannot know it exists or what it does.
    This caught OPENAI_API_KEY, added to compose without being documented.
    """
    declared = _declared_vars()
    compose = _compose()
...
def test_openwebui_routes_through_the_monitor() -> None:
    """The whole lab depends on chat traffic traversing the guardrail.

    OpenWebUI was configured with OLLAMA_BASE_URL=http://ollama:11434, so every
    request from the UI bypassed classification, blocking and telemetry. The
    monitor only ever saw the simulation scripts.
    """
    compose = _compose()
...
def test_proxy_exposes_the_endpoints_openwebui_needs() -> None:
    """The OpenAI-compatible connection needs a model list, not just completions.

    Without GET /v1/models the front-end's model picker is empty and the
...
# ---------------------------------------------------------------------------
# Playbook capability claims
# ---------------------------------------------------------------------------


def test_playbooks_do_not_document_nonexistent_endpoints() -> None:
    """A playbook must not tell an analyst to call an API that does not exist.

    Three playbooks instructed containment via "block the session at the proxy",
    but the proxy exposes no such endpoint. Containment has to happen at a layer
    that already has identity.
    """
    src = (ROOT / "llm-monitor" / "proxy.py").read_text(encoding="utf-8")
    exposed = set(re.findall(r'@app\.(?:get|post|put|delete)\("([^"]+)"', src))

    bad: list[str] = []
    for pb in sorted((ROOT / "playbooks").glob("*.md")):
        text = pb.read_text(encoding="utf-8")
        for method, path in re.findall(r"\b(GET|POST|PUT|DELETE)\s+(/[A-Za-z0-9_/.{}-]+)", text):
            if path in exposed:
                continue
            # A mention that explicitly says the endpoint does not exist is
            # documentation of a gap, which is what we want.
            window = text[max(0, text.index(path) - 220) : text.index(path) + 220].lower()
            if any(w in window for w in ("does not exist", "no session-blocking", "not exist")):
                continue
            bad.append(f"{pb.name}: {method} {path}")

    assert not bad, "playbooks reference endpoints the proxy does not expose:\n  " + "\n  ".join(
        bad
    )


def test_exfiltration_rule_survives_redaction() -> None:
    """The default configuration must not silently disable a detection.

    REDACT_LOGGED_SECRETS defaults to true, which masks the completion text.
    data_exfiltration_via_llm keys primarily on secrets_in_completion so it still
    fires; this asserts that primary selection is present.
    """
    rule = yaml.safe_load(
        (ROOT / "detections" / "sigma" / "data_exfiltration_via_llm.yml").read_text(
            encoding="utf-8"
        )
    )
    det = rule["detection"]
    assert "selection_structured" in det, "rule lost its redaction-proof selection"
    structured = det["selection_structured"]
    # Sigma keys carry a modifier suffix, e.g. "secrets_in_completion|contains".
    assert any(k.split("|")[0] == "secrets_in_completion" for k in structured), (
        f"structured selection does not key on secrets_in_completion: {list(structured)}"
    )
    assert det["condition"].startswith("1 of selection"), (
        f"condition {det['condition']!r} would not fire on either selection"
    )


def test_redaction_is_on_by_default() -> None:
    """Secure by default: a prompt can carry a real credential.

    The audit log is shipped to a search cluster with a wider audience than the
    model runtime, so masking must be the default rather than an opt-in.
    """
    src = (ROOT / "llm-monitor" / "proxy.py").read_text(encoding="utf-8")
    m = re.search(r'os\.getenv\("REDACT_LOGGED_SECRETS",\s*"(\w+)"\)', src)
    assert m, "REDACT_LOGGED_SECRETS default not found"
    assert m.group(1) == "true", f"redaction defaults to {m.group(1)!r}, expected 'true'"

    assert _declared_vars()["REDACT_LOGGED_SECRETS"] == "true"
